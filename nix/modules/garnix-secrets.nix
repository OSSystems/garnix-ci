...
let
  ...
  pathOption = description: lib.mkOption {
    ...
    default = null;
    description = description + " (string path. Do NOT use a Nix path literal or the secret will be copied into the world-readable /nix/store).";
  };

  installedSecrets = lib.filter (s: s.sourcePath != null) [
    { name = "database-password"; sourcePath = secretsCfg.databasePasswordPath; required = true; }
    { name = "github_webhook_secret"; sourcePath = secretsCfg.githubWebhookSecretPath; required = true; }
    { name = "github_client_secret"; sourcePath = secretsCfg.githubClientSecretPath; required = true; }
    { name = "github_client_id"; sourcePath = secretsCfg.githubClientIdPath; required = true; }
    { name = "github_app_id"; sourcePath = secretsCfg.githubAppIdPath; required = true; }
    { name = "github_app_pk"; sourcePath = secretsCfg.githubAppPkPath; required = true; }
    { name = "opensearch-garnix"; sourcePath = secretsCfg.opensearchCredentialPath; required = true; }
    { name = "garnix-jwt-key"; sourcePath = secretsCfg.jwtKeyPath; required = true; }
    { name = "repo-secrets-key"; sourcePath = secretsCfg.repoSecretsKeyPath; required = true; }
    { name = "repo-secrets-key-pub"; sourcePath = secretsCfg.repoSecretsPubKeyPath; required = true; }
  ...
  requiredSecrets = [
    { option = "secrets.databasePasswordPath"; value = secretsCfg.databasePasswordPath; required = true; }
    { option = "secrets.githubWebhookSecretPath"; value = secretsCfg.githubWebhookSecretPath; required = true; }
    { option = "secrets.githubClientSecretPath"; value = secretsCfg.githubClientSecretPath; required = true; }
    { option = "secrets.githubClientIdPath"; value = secretsCfg.githubClientIdPath; required = true; }
    { option = "secrets.githubAppIdPath"; value = secretsCfg.githubAppIdPath; required = true; }
    { option = "secrets.githubAppPkPath"; value = secretsCfg.githubAppPkPath; required = true; }
    { option = "secrets.opensearchCredentialPath"; value = secretsCfg.opensearchCredentialPath; required = true; }
    { option = "secrets.jwtKeyPath"; value = secretsCfg.jwtKeyPath; required = true; }
    { option = "secrets.repoSecretsKeyPath"; value = secretsCfg.repoSecretsKeyPath; required = true; }
    { option = "secrets.repoSecretsPubKeyPath"; value = secretsCfg.repoSecretsPubKeyPath; required = true; }
    { option = "secrets.s3CacheAccessKeyIdPath"; value = secretsCfg.s3CacheAccessKeyIdPath; required = cfg.s3Cache.enable; }
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    assertions = lib.optionals (!config.garnix.devMode.enable) (lib.map
      (s: {
    ...
    systemd.services.garnix-secrets-stage = {
      description = "Stage garnix secrets into /run/secrets";
      wantedBy = [ "multi-user.target" ];
      before = [ "garnixServer.service" "frontend.service" "fluent-bit.service" ];
